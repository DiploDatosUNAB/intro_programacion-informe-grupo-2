...
Una vez filtrado por edad, nos interesa conocer la cantidad de víctimas por edad y por sexo.
```{r}
victimas_parque %>% 
  filter(edad != "-") %>% 
  group_by(edad, sexo) %>% 
...
Exploramos por ocupación y sexo, filtrando las ocupaciones con cantidad de víctimas mayor a 50. El motivo de la elección surge de buscar qué ocupaciones eran más relevantes y si estas tienen una relación con la militancia.
...
Ahora procedemos a eliminar todos los datos faltantes (NA).
...
El dataframe contiene 4156 observaciones; es decir; 4156 víctimas con pertenencia algún espacio de militancia.
```{r}
glimpse(militancias)
```

Procedemos a conocer la cantidad de víctimas con afiliación a espación de militancia.
...
Analizamos la cantidad de víctimas con ocupacion Estudiante.
```{r}
victimas_parque %>% 
  group_by(fecha_de_secuestro) %>% 
  mutate(fecha = dmy(fecha_de_secuestro)) %>% 
...
Durante los primeros años del golpe de Estado cívico-militar, observamos que la mayor cantidad de secuestros y/o asesinatos de víctimas ocurrieron entre los años 1976 y 1977, y el rango etario predominante de las víctimas fue de 20 a 30 años. Esto nos hace reflexionar si la edad universitaria fue un factor a la hora de convertirse en posibles víctimas. También nos preguntamos cómo varió la edad y ocupación de las víctimas entre los años 1978 y 1982. Si disminuyó el número de secuestros y/o asesinatos, pensar en posibles respuestas que indiquen esta característica.
...
De acuerdo a lo analizado, la ocupación Estudiante agrupa una gran cantidad de víctimas, pero no es la que mayor cantidad de víctimas representa. Agrupando por edad, por año, posee mayor número de víctimas en conjunto que las demás ocupaciones. Lo que resulta de este análisis es que si bien es una variable identiraria representativa, no es la única y no necesariamente la edad universitaria fue un factor a la hora de convertirse en posibles víctimas. 
...
De 4276 victimas del 76 al 82, 527 son de actividad sindical, 1147 estudiantes, las 2602 que no son estudiantes ni sindicalistas,  cuantas son y a qué pertenecen?
```{r}
filtro_ms %>% 
  filter(anio %in% c(1976:1982)) %>% 
...
##Conclusión: decidir que hacer con el grafico
...
filtro_lugar %>% 
  filter(anio %in% c(1976:1982)) %>%
  filter(lugar_de_secuestro == "TUCUMAN") %>% 
  group_by(lugar_de_secuestro, orga1, sindicato) %>% 
  filter(sindicato == "sindicalista") %>% 
  summarise(n = n()) %>% 
  arrange(desc(n))
```
Sindicalistas CPAITAL FEDERAL
```{r}
filtro_lugar %>% 
  filter(anio %in% c(1976:1982)) %>%
