...
Nos llamó la atención la cantidad de víctimas en Mar de Plata, por lo que buscamos las ocupaciones de las mismas en el lugar.
```{r}
victimas_parque %>% 
  filter(!is.na(lugar_de_secuestro)) %>% 
...
```
Los datos anteriores no respondieron lo que buscabamos, y decidimos filtrar por Mar del Plata con los espacios de militancia de las víctimas.
```{r}
victimas_parque %>% 
  filter(!is.na(lugar_de_secuestro)) %>% 
...
Queremos conocer si las observaciones de victimas_parque mantienen relación con las del dataframe victimas_denuncia.
...
Filtramos los lugares de secuestro de más de 100 victimas, dejando de lado los datos faltantes.
```{r}
victimas_denuncia %>% 
  filter(!is.na(lugar_detencion_secuestro)) %>% 
  group_by(lugar_detencion_secuestro) %>% 
...
ocupacion, edad y lugar de secuestro

lugar del secuestro/detencion, ocupacion, militancia

tipificacion del hecho, lugar secuestro/detencion y edad
